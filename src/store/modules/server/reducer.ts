...
produce(state, (draftState) => {
  ...
  draftState.config.DisableDefaultNet = action.payload.DisableDefaultNet === "on"
  draftState.config.DisableRemoteIPCheck = action.payload.DisableRemoteIPCheck === "on"
  draftState.config.GRPCConnString = action.payload.GRPCConnString
  draftState.config.GRPCHost = action.payload.GRPCHost
  draftState.config.GRPCPort = Number.parseInt(action.payload.GRPCPort) || -1
