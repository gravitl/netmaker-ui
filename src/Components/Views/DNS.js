...
export default function DNS({ data, nodes, user }) {
  ...
  const removeEntry = async (name) => {
    if (
      data &&
      window.confirm(
        `Are you sure you want to remove entry for ${name}.${data.netid}?`
      )
    ) {
      const dnsResponse = await DNS_API.removeEntry(
        data.netid,
        name,
        user.token
      )
            ...
            <Grid item xs={12}>
              <div className={classes.titleWithButton}>
                <Tooltip title={`Add DNS entry for network, ${data.netid}?`}>
                  <Button
                    style={{ width: '33%', marginBottom: '0.5em' }}
                    variant="outlined"
                    onClick={handleOpen}
                  >
                    Add Entry{'  '}
                    ...
                    {dnsData.map((dnsEntry) => (
                      <StyledTableRow key={dnsEntry.name}>
                        ...
                        <StyledTableCell align="center">
                          <Tooltip
                            title={`REMOVE DNS ENTRY: ${dnsEntry.name}.${dnsEntry.network}?`}
                            placement="top"
                          >
                            <IconButton
                              style={{ height: '100%' }}
                              onClick={() =>
                                removeEntry(dnsEntry.name, dnsEntry.address)
                              }
                            >
                              <Backspace />
                            </IconButton>
