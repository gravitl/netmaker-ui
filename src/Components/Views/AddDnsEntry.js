...
export default function AddDnsEntry({
  setOpen,
  networkName,
  nodes,
  netid,
  ...
  const handleSubmit = async () => {
    ...
    if (status) {
      ...
      const response = await DNS_API.createEntry(
        networkName,
        dnsName,
        address,
        user.token
      ...
      if (response) {
        setSuccess(
          `Successfully added DNS entry with name ${dnsName} for address: ${address}`
        )
        setDnsName('')
        setAddress('')
        ...
        setError(
          'Could not complete request, please check connections and try again.'
        )
      }
      ...
      if (cause === 'subnet')
        setError(
          'Invalid address provided. Please check formatting. ex: "192.168.1.69"'
        )
      else setError('Invalid DNS entry name provided. Max 192 characters.')
    }
      ...
      <Grid item xs={8}>
        ...
        >
          <Grid container className={classes.container}>
            <Grid item xs={1}>
              <Tooltip
                title="This is the address of the machine, may be an ip or domain like gravitl.com."
                placement="top"
              >
            ...
            <Grid item xs={1}>
              <Tooltip
                title="The dns entry you would like for the machine, typically something.com or whatever.net, etc."
                placement="top"
              >
