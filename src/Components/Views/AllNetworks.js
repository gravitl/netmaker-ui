...
export default function AllNetworks({
  ...
  const handlePublicKeyRefresh = async (networkName, displayName) => {
    try {
      if (
        window.confirm(
          `Are you sure you want to refresh the node public keys on network: ${displayName}?`
        )
      ) {
        const response = await API(user.token).post(
          `/networks/${networkName}/keyupdate`
        )
  ...
  return networks[selectedNetwork] ? (
    <NetworkDetails
      ...
      networkData={networks[selectedNetwork]}
    ...
    >
      <Grid item xs={11}>
        ...
        <div className={classes.nodeTitle2}>
          <Typography variant="h5">All networks</Typography>
        </div>
          ...
          networks.map((network, i) => (
            <Card key={i} className={classes.row}>
              <CardHeader
                ...
                action={
                  <Tooltip
                    title={`EDIT ${network.displayname}`}
                    placement="top"
                  >
                    <IconButton
                      aria-label={`edit network ${network.displayname}`}
              ...
              <CardContent>
                <Grid container>
                  <Grid item className={classes.buttonContainer} xs={4} md={2}>
                    <Tooltip
                      title={`Refresh Public Keys for ${network.displayname}`}
                      placement="top"
                    >
                      <IconButton
                        aria-label={`refresh public keys ${network.displayname}`}
                        onClick={() =>
                          handlePublicKeyRefresh(
                            network.netid,
                            network.displayname
                          )
                        }
                  ...
                  {displayedNetworkFields.map((fieldName, j) => (
                    <Grid
                      ...
                      key={j}
                      xs={fieldName === 'addressrange' ? 8 : 12}
                      md={3}
                    >
                      <TextField
                        ...
                        placeholder={
                          fieldName === 'addressrange'
                            ? network[fieldName].toString()
                            : Util.timeConverter(network[fieldName].toString())
                        }
                        key={fieldName}
                        fullWidth
                        disabled
