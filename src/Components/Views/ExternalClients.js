...
export default function ExternalClients({ data, isAllNetworks, nodes, user }) {
  ...
  const handleUpdateClient = async (client, clientid) => {
    ...
    try {
      const response = await API(user.token).put(
        `/extclients/${client.network}/${client.clientid}`,
        { clientid }
      )
      if (response.status === 200) {
        fetchExternals()
        setSuccess(
          `Successfully updated client, ${client.clientid}, to ${clientid} on network, ${client.network}.`
        )
      } else {
  ...
  const getQrCodes = (clients) => {
    try {
      ...
      clients.map(async (client) => {
        const response = await API(user.token).get(
          `/extclients/${client.network}/${client.clientid}/qr`,
  ...
  const createNewClient = async (network, macaddress) => {
    if (
      window.confirm(
        `Are you sure you want to add an external client to network ${network}?`
      )
    ) {
      setIsProcessing(true)
      setError('')
  ...
  const getConfigFile = async (client) => {
    ...
    try {
      const response = await API(user.token).get(
        `/extclients/${client.network}/${client.clientid}/file`,
        { responseType: 'blob' }
      )
      if (response.status === 200) {
      ...
      setError(
        `Server error occurred when fetching config file for client, ${client.clientid}!`
      )
      setTimeout(() => {
        setError('')
      }, 1500)
  ...
  const deleteClient = async (client) => {
    if (
      window.confirm(
        `Are you sure you want to remove client: ${client.clientid} from network ${client.network}?`
      )
    ) {
      setIsProcessing(true)
      try {
        ...
        if (response.status === 200) {
          setSuccess(
            `Succesfully Removed Client: ${client.clientid} from network ${client.network}.`
          )
          fetchExternals()
        } else {
  ...
  React.useEffect(() => {
    if (
      (data != null && externals.length === 0 && !hasChecked) ||
      (data != null && !hasChecked)
    ) {
      fetchExternals()
    }
  })
        ...
        )}
        {
          <Grid item xs={12}>
            <Grid container justifyContent="flex-start" alignItems="stretch">
              <Grid item xs={4}>
                <div className={classes.center}>
                  <h4>Available Ingress Gateways</h4>
                </div>
                <div className={classes.cardContainer}>
                  {nodes.map((node) =>
                    node.isingressgateway === 'yes' &&
                    (isAllNetworks || node.network === data.netid) ? (
                      <Card
                        key={node.name + ':' + node.macaddress}
                        className={classes.cardBasic}
                      >
                        <CardHeader
                          title={node.name}
                          subheader={`Network: ${node.network}`}
                          avatar={<SettingsInputAntenna />}
                          action={
                            <Tooltip
                              title={`ADD CLIENT TO NETWORK ${node.network}?`}
                              placement="top"
                            >
                              <IconButton
                                aria-label={`add client to network, ${node.network}`}
                                onClick={() =>
                                  createNewClient(node.network, node.macaddress)
                                }
                              >
                  ...
                  {nodes.filter(
                    ...
                    <div className={classes.center}>
                      <h5>No ingress gateways present.</h5>
                    </div>
                  ) : null}
                </div>
              </Grid>
                ...
                <div className={classes.cardContainer}>
                  {externals.length
                    ? externals.map((external, i) =>
                        data &&
                        (isAllNetworks || external.network === data.netid) ? (
                          <Accordion
                            square={false}
                            key={i}
                            expanded={currentClient === external.clientid}
                            onChange={handleAccordianChange(external.clientid)}
                            style={
                              i === externals.length - 1
                                ? { marginBottom: '6em' }
                                : {}
                            }
                          >
                            ...
                            <AccordionDetails>
                              <Card className={classes.cardBasic}>
                                <CardHeader
                                  ...
                                  action={
                                    <Tooltip
                                      title={`EDIT CLIENT ${external.clientid}`}
                                      placement="top"
                                    >
                                      <IconButton
                                        aria-label={`edit external client ${external.clientid}`}
                                        onClick={() =>
                                          openModal(false, null, external)
                                        }
                                      >
                                        <Edit />
                                      </IconButton>
                                ...
                                <CardContent>
                                  ...
                                  >
                                    ...
                                    >
                                      <h5>
                                        IP - <strong>{external.address}</strong>
                                      </h5>
                                      <h5 style={{ wordWrap: 'break-word' }}>
                                        Public Key -{' '}
                                        <strong>{external.publickey}</strong>
                                      </h5>
                                      <div
                                        style={{
                                          display: 'flex',
                                      ...
                                      >
                                        <Button
                                          variant="contained"
                                          onClick={() =>
                                            getConfigFile(external)
                                          }
                                        >
                                          Download Config
                                        </Button>
                                        <Tooltip
                                          title={`DELETE CLIENT ${external.clientid}?`}
                                          placement="top"
                                        >
                                          <IconButton
                                            aria-label={`delete external client ${external.clientid}`}
                                            onClick={() =>
                                              deleteClient(external)
                                            }
                                          >
                                      ...
                                      <div className={classes.center}>
                                        {qrCodes && qrCodes != {} ? (
                                          <img
                                            ...
                                            src={qrCodes[external.clientid]}
                                            alt={`Small QR code for client, ${external.clientid}.`}
                                            onClick={() =>
                                              openModal(
                                                true,
                                                qrCodes[external.clientid],
                                                external
                                              )
                                            }
                                          />
                    ...
                    : null}
                  {externals.filter(
                    (client) => isAllNetworks || client.network === data.netid
                  ).length === 0 ? (
                    <div className={classes.center}>
                      <h5>No External clients present.</h5>
                    </div>
                  ) : null}
                </div>
              </Grid>
