...
const readOnlyFields = ['nodeslastmodified', 'networklastmodified']

const boolFields = ['defaultsaveconfig', 'isdualstack', 'defaultudpholepunch']
...
const timeFields = ['nodeslastmodified', 'networklastmodified']
...
export default function NetworkDetails({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (
        !(
          settings.defaultextclientdns === '' ||
          correctIPRegex.test(settings.defaultextclientdns)
        )
      ) {
      ...
      const response = await API(user.token).put(
        `/networks/${networkData.netid}`,
        { ...settings, allowmanualsignup: allowManual ? 'yes' : 'no' }
      )
  ...
  const removeNetwork = async (networkName) => {
    if (
      window.confirm(`Are you sure you want to remove network ${networkName}?`)
    ) {
      setIsProcessing(true)
      try {
        ...
        if (response.status === 200) {
          ...
          setError(
            `Could not remove network: ${networkName}, please remove all nodes and try again.`
          )
        }
      } catch (err) {
        setError(
          `Server error occurred when removing: ${networkName}, please check connection and try again.`
        )
      }
    }
    setTimeout(() => {
  ...
  const IS_UDP_ENABLED = (field) =>
    config && config.ClientMode == 'off' && field === 'defaultudpholepunch'
    ...
    <div>
      <form>
        ...
        >
          ...
          <Grid item xs={12}>
            <div className={classes.buttons}>
              ...
              <Button
                ...
                variant="outlined"
                onClick={() => removeNetwork(networkData.netid)}
              >
                Delete
              </Button>
              {back && (
          ...
          {networkData &&
            settings &&
            Fields.NETWORK_FIELDS.map((fieldName) => {
                  ...
                  {boolFields.indexOf(fieldName) >= 0 ? (
                    ...
                    <TextField
                      ...
                      disabled={
                        !isEditing ||
                        readOnlyFields.indexOf(fieldName) >= 0 ||
                        (settings.isdualstack === 'no' &&
                          fieldName === 'addressrange6')
                      }
                      margin="normal"
                      InputLabelProps={{
                        shrink: true,
