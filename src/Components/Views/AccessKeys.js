...
export default function OtkDetails({ data, user }) {
      ...
      if (window.confirm(`Are you sure you want to remove key: ${keyName}?`)) {
        ...
        try {
          const response = await API(user.token).delete(
            `/networks/${data.netid}/keys/${keyName}`
          )
          if (response.status === 200) {
  ...
  React.useEffect(() => {
    if (
      (data != null && accessKeys && accessKeys.length === 0 && !hasChecked) ||
        ...
        <div className={classes.paper}>
          <Card className={classes.center}>
            <CardContent>
              ...
              <h5>
                {keyText}
                <Tooltip title={'COPY ACCESS KEY'} placement="top">
                  <IconButton variant="outlined" onClick={() => copy(keyText)}>
                    <FilterNone />
                  </IconButton>
                </Tooltip>
              </h5>
              ...
              <h6>
                ...
                <Tooltip title={'COPY AGENT INSTALL COMMAND'} placement="top">
                  <IconButton
                    variant="outlined"
                    onClick={() => copy(getAgentInstallCommand())}
                  >
                    <FilterNone />
                  </IconButton>
                </Tooltip>
                        ...
                        <Grid container justify="center" alignItems="center">
                          ...
                          <Grid item xs={4} className={classes.center}>
                            <Button
                              ...
                              onClick={() => deleteKey(accessKey.name)}
                              variant="outlined"
                            >
                              Delete Key <Delete />
                            </Button>
