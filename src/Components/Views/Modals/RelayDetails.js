...
export default function RelayDetails({
  handleClose,
  setSuccess,
  addRelay,
  relayNode,
    ...
    <Grid container xs={12} justify="center" className={classes.mainContainer}>
      ...
      <Grid item xs={12}>
        <h3 style={{ textAlign: 'center' }}>
          Creating Relay from {relayNode.name}
        </h3>
        <form
          className={classes.form}
          onSubmit={(e) => {
            e.preventDefault()
            addRelay(relayNode, selectedNodes)
          }}
        >
          <Grid container justifyContent="center" alignItems="center">
            <Grid item xs={8} md={4}>
              <FormControl className={classes.formControl} fullWidth>
                <InputLabel id="nodes-checkbox-label">
                  Select Node Addresses
                </InputLabel>
                <Select
                  ...
                  {nodeData && nodeData.length
                    ? nodeData.map((node) => (
                        <MenuItem key={node.name} value={node.address}>
                          <Checkbox
                            color="primary"
                            checked={selectedNodes.indexOf(node.address) >= 0}
                          />
