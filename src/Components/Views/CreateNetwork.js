...
export default function CreateNetwork({
  ...
  const correctNetworkNameRegex = new RegExp(
    /^[a-z0-9,\-,_,a-z0-9]{1,12}[\.,a-z0-9]*$/i
  )
  ...
  const handleSubmit = async () => {
    ...
    if (status) {
      ...
      if (cause === 'subnet')
        setError(
          'Invalid subnet provided. Please check formatting. ex: 192.168.1.10/23.'
        )
      else if (cause === 'local')
        setError(
          'Invalid local address provided. Please check formatting. ex: 192.168.1.10/23.'
        )
      else
        setError(
          'Invalid network name provided. Max 12 alphanumeric characters with underscores or hyphens.'
        )
    }
  }
