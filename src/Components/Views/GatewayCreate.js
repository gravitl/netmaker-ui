...
export default function CreateGateway({
  ...
  const correctInterfaceNameRegex = new RegExp(
    /^[a-zA-Z0-9,\-,_,a-zA-Z0-9]{3,15}$/i
  )
  ...
  const handleSubmit = async () => {
    ...
    if (status) {
      ...
      const response = await API(user.token).post(
        `/nodes/${gatewayNode.network}/${gatewayNode.macaddress}/creategateway`,
        {
          ranges: newRanges,
      ...
      if (response.status === 200) {
        setSuccess(
          `Successfully created gateway with node ${gatewayNode.name} for address range: ${addressranges}`
        )
        setTimeout(() => {
          setSuccess('')
          setOpen(false)
      ...
      if (cause === 'subnet')
        setError(
          `Invalid address range(s) provided. Please check formatting of entry #${entry}. ex: 192.168.1.0/24.`
        )
      else if (cause === 'dns') {
        setError(`Invalid DNS name given for entry #${entry}. (ex: hello.net)`)
      } else
        setError(
          'Invalid network interface provided. Max 15 alphanumeric characters with underscores [_] or hyphens [-].'
        )
    }
  }
      ...
      <Grid item xs={8}>
        ...
        >
          {networks.map((network) => (
            <MenuItem
              onClick={(event) => handleNetworkSelect(event, network)}
              key={network.netid}
            >
              {network.displayname}
            </MenuItem>
      ...
      <Grid item xs={8}>
        ...
        >
          <Grid container className={classes.container}>
            <Grid item xs={1}>
              <Tooltip
                title="This is the IP Address  Range that this node will forward traffic to. for instance, your office LAN. This will typically be a 172.x.x.x or 192.x.x.x network, or one of your other Netmaker networks. We recommend using a Netmaker network,  but make sure this node is in it!"
                placement="top"
              >
            ...
            <Grid item xs={1}>
              <Tooltip
                title='The network interface that this node uses (locally) to reach the address range provided above. For instance, "eth0". If using a This node will forward traffic to the address range specified using this interface.'
                placement="top"
              >
                <IconButton aria-label={`set address range for gateway node.`}>
                  <Info />
