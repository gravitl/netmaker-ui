  ...
  const validate = () => {
    ...
    if (!deleteMode) {
      ...
      const isConfirmPassword =
        correctPasswordRegex.test(confirmPassword) &&
  ...
  const handleSubmit = async () => {
    ...
    if (status) {
      ...
      if (deleteMode) {
        ...
        if (isAdmin || isCreatingAdmin) {
          ...
          if (selectedNetworks.length > 0)
            response = await USER.createRegularUser(
              currentUser.token,
              userName,
              password,
              selectedNetworks
      ...
      if (response.status === 200) {
        setSuccess(
          `Successfully ${deleteMode ? 'deleted' : 'created'} user ${userName}.`
        )
        setIsCreating(false)
    ...
    <Grid container xs={12} justify="center" className={classes.mainContainer}>
      ...
      <Grid item xs={8}>
        ...
        <h3 style={{ textAlign: 'center' }}>
          {!deleteMode
            ? `Create a new ${isAdmin || isCreatingAdmin ? 'admin' : 'user'}.`
            : `Delete a user`}
        </h3>
        <form
                ...
                <div className={classes.currentUserBox}>
                        ...
                        <Checkbox
                          onChange={() => {
                            if (
                              !isCreatingAdmin &&
                              window.confirm(
                                'Are you sure? By doing this, you will give the user admin rights over your NetMaker server.'
                              )
                            )
                              setIsCreatingAdmin(true)
                            else setIsCreatingAdmin(false)
                          }}
                  ...
                  {isCreatingAdmin ? null : (
                    <FormControl className={classes.formControl} fullWidth>
                      <InputLabel id="networks-checkbox-label">
                        ...
                        {networks
                          ? networks.map((name) => (
                              <MenuItem key={name} value={name}>
                                <Checkbox
                                  color="primary"
                                  checked={selectedNetworks.indexOf(name) > -1}
                                />
