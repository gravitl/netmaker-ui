...
const columns: TableColumns<Node> = [
  {
    ...
    align: 'center',
    sortable: true,
    format: (_, node) => <Typography variant='body1'>
...
const dnsColumns: TableColumns<DNS> = [
  {
    ...
    align: 'center',
    sortable: true,
    format: (_, entry) => <Typography variant='body1'>
...
export const DNSView: React.FC = () => {
  ...
  // const entries = 
  const customEntries = filterCustomDNSByNetwork(nodes as Node[], currentEntries, netid)
  useLinkBreadcrumb({
    link: url,
    title: netid,
  })
  ...
  if (!nodes.length) {
    ...
    >
      ...
      <Grid item xs={12} sx={{margin: '0.5em 0em 1em 0em'}}>
        <div style={centerText as any}><h3>{t('dns.none')}</h3></div>
      </Grid>
    </Grid>
  }
  ...
  const handleDeleteEntry = () => {
    dispatch(deleteDnsEntry.request({
      domain: selected,
      netid,
    }))
  }
    ...
    <Switch>
      <Route exact path={path}>
        ...
        >
          ...
          <Grid item xs={12} sm={12} md={5} sx={{margin: '0.5em 0.5em 0.5em 0.5em'}}>
          ...
          <Grid item xs={12} sm={12} md={6} sx={{margin: '0.5em 0.5em 0.5em 0.5em'}}>
              <Grid container>
                  <Grid item xs={8}>
                    <div style={centerText as any}>
                        <Typography variant='h4' sx={{marginBottom: '1em'}}>
                    ...
                    <Grid item xs={4}>
                        <Button color='primary' variant='contained' component={Link} to={`${url}/create`} >
                            {t('dns.create')}
                        </Button>
                    </Grid>
                    <NmTable
                        ...
                        actions={[(row) => ({
                          tooltip: `${t('common.delete')} : ${row.name}.${row.network}`,
                          disabled: false,
                          icon: <Delete />,
                          onClick: () => {
                            handleOpen(row.name)
