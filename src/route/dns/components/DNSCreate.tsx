...
export const DNSEntryCreate: React.FC<{
      ...
      const nodeAddresses = listOfNodes.map(node => `${node.address} ${node.name}`)
    ...
    const handleSelection = (newAddress: string) => {
        formRef.current?.reset({...formRef.current?.values, address: newAddress}, { keepDefaultValues: false})
    }
    ...
    return (<Modal
      ...
      >
        <Box style={boxStyle.modal}>
            <Grid container justifyContent='center' alignItems='center' style={{margin: '0.5em'}}>
